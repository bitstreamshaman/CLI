...
class CLIController:
    ...
    def __init__(
        self, agent=None, console: Optional[Console] = None, debug_mode: bool = False
    ):
        """
    ...
    def _initialize_handlers(self):
        ...
        try:
            ...
            self.control_handler = ControlCommandHandler(
                self.shell_executor, self.console
            )
            self.shell_handler = ShellCommandHandler(
                self.agent, self.shell_executor, self.shell_detector, self.console
            )
            self.ai_handler = AIRequestHandler(
                self.agent, self.shell_executor, self.console
            )
    ...
    def _initialize_processors(self):
        ...
        try:
            ...
            raise CLIInitializationError(
                f"Failed to initialize command processor: {e}"
            ) from e
    ...
    def _process_command(self, user_input: str):
        ...
        try:
            ...
            self.console.print(
                f"[red]❌ Unexpected error processing command: {e}[/red]"
            )
            if self.debug_mode:
                self.console.print_exception()
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"CLIController(running={self.running}, "
...
def create_cli_controller(
    agent=None, console: Optional[Console] = None, debug_mode: bool = False
) -> CLIController:
    """
