...
def prompt_for_setup():
    ...
    print_console()
    response = (
        input("Would you like to set up your API key now? (y/n): ").strip().lower()
    )
    ...
    if response in ["y", "yes", ""]:
        ...
        print_console(
            "[yellow]💡 You can run the setup later or set the API key manually.[/yellow]"
        )
        print_console(
            "[blue]Manual setup:[/blue] echo 'ANTHROPIC_API_KEY=your_key' > ~/.ifw.env"
        )
        return None
