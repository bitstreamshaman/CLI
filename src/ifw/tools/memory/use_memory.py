        ...
        if isinstance(result, dict) and result.get('status') == 'success':
            ...
            try:
                ...
                if relevant_memories:
                    ...
                    sorted_memories = sorted(relevant_memories, key=lambda x: x.get('score', float('inf')))
                    top_memories = sorted_memories[:5]
                    
                    memories_context = "\n".join([f"- {mem['memory']}" for mem in top_memories])
        ...
        if (isinstance(result, dict) and result.get('status') == 'success') or \
           (hasattr(result, 'status') and result.status == "success"):
            try:
                ...
                recent_memories = memories_data[-limit:] if len(memories_data) > limit else memories_data
                ...
                return f"Recent memories for {user_id} ({len(recent_memories)} of {len(memories_data)} total):\n" + "\n".join(formatted)
